...
final_grades |>
  mutate(group = factor(group,
                        levels = sort(unique(as.integer(as.character(group)))))) |>
  ggplot(aes(x = sex, y = final_grade)) +
  geom_boxplot() +
...
avg_grades_per_student <-
  ...
  summarise(
    mean_grade = mean(grade),
    .by = c(id, group, semester, course)
  )

pass_df <-
  avg_grades_per_student |>
  summarise(
    final_grade = mean(mean_grade),
    min_course  = min(mean_grade),
    .by = c(id, group)
  ) |>
  left_join(
    avg_grades_per_student |>
      summarise(sem_mean = mean(mean_grade),
                .by = c(id, group, semester)) |>
      tidyr::pivot_wider(
        names_from  = semester,
        values_from = sem_mean,
        names_prefix = "sem"
      ),
    by = join_by(id, group)
  ) |>
...
not_pass <- pass_df |>
  ...
  summarise(n = n()) 
```
The number of students having a final grade equal or larger than 10 that didn't pass is **`r not_pass`**.
